...
- `runcoms/zshrc` order: `zshenv → lib/options → lib/history → lib/colors →
  ...
  `lib/completion`, which feeds it into the completion menu's list-colors; it also sets
  `$GREP_COLORS`, `$LESS_TERMCAP` for man, and BSD `$LSCOLORS`. The rich `$LS_COLORS`
  (500+ types) is generated by **vivid** from `integrations/vivid/kronuz.yml` into the
  committed `integrations/vivid/ls_colors` — regenerate after editing the theme:
  `cp integrations/vivid/kronuz.yml ~/.config/vivid/themes/kronuz.yml && vivid generate
  kronuz > integrations/vivid/ls_colors`. Its filekinds match `integrations/eza/theme.yml`,
  which `$LS_COLORS` overrides in eza, so keep the two in sync.)
